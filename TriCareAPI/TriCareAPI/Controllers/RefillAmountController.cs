...
{
    //[Authorize]
    public class RefillAmountController : ApiController
    {
        // GET api/values
        public string Get()
        {
            var util = new RefillUtil(new TriCareDataDataContext());
            var result = util.ConvertListToAmountModel(util.GetRefillAmounts());
        ...
        {
            var util = new RefillUtil(new TriCareDataDataContext());
            var result = util.ConvertToAmountModel(util.GetRefillAmount(id));
            var json = new JavaScriptSerializer().Serialize(result);
            return json;
        }
